...
async def process_blocks_for_masternode_transactions(force_recheck_from_scratch=False):
    while True:
        await asyncio.sleep(0.2)  # Sleep to yield control
        global rpc_connection
        ...
        for start_block in range(last_processed_block + 1, latest_block_height + 1, batch_size):
            ...
            if end_block % log_interval == 0:
                logger.info(f"Processing blocks {end_block - log_interval:,} to {end_block:,} for masternode transactions...")
            tasks = []
            for block_height in range(start_block, end_block + 1):
                tasks.append(asyncio.create_task(process_block(block_height, masternode_transactions_dict)))
            await asyncio.gather(*tasks)
            await asyncio.sleep(random.uniform(0.1, 0.2))
            if end_block % save_interval == 0 or end_block == latest_block_height:
                save_start_block = (end_block // save_interval) * save_interval + 1
                save_end_block = end_block
                save_path = os.path.join('masternode_transactions_dicts', f"masternode_txids__block_{save_start_block}_to_block_{save_end_block}.pickle")
                with open(save_path, 'wb') as f:
                    pickle.dump(masternode_transactions_dict, f)
                logger.info(f"Saved masternode transactions from block {save_start_block:,} to {save_end_block:,}")
                masternode_transactions_dict = {}
